...
def build(ctx):
    ...
    subprocess.run(['make', '-j4', '-C', 'mpy-cross'], cwd=lv_micropython_dir)
    subprocess.run(['make', '-j4', '-C', 'ports/unix', 'submodules'], cwd=lv_micropython_dir)
    subprocess.run(['make', '-j4', '-C', 'ports/unix'], cwd=lv_micropython_dir)
